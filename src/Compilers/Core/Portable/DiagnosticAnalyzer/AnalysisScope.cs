...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    ...
    {
        public SourceOrNonSourceFile? FilterTreeOpt { get; }
        ...
        {
            return new AnalysisScope(SyntaxTrees, NonSourceFiles, analyzers, FilterTreeOpt, FilterSpanOpt, IsSyntaxOnlyTreeAnalysis, ConcurrentAnalysis, CategorizeDiagnostics);
        }
        ...
        {
            return FilterTreeOpt == null || FilterTreeOpt.SourceTree == tree;
        }

        public bool ShouldAnalyze(AdditionalText file)
        {
            return FilterTreeOpt == null || FilterTreeOpt.NonSourceFile == file;
        }
        ...
        public bool ShouldAnalyze(ISymbol symbol)
        {
            ...
            if (FilterTreeOpt.SourceTree == null)
            {
                return false;
            }
            ...
            {
                if (location.SourceTree != null && FilterTreeOpt.SourceTree == location.SourceTree && ShouldInclude(location.SourceSpan))
                {
                    return true;
                }
            }
        ...
        public bool ShouldInclude(Diagnostic diagnostic)
        {
            ...
            {
                if (FilterTreeOpt?.SourceTree == null ||
                    diagnostic.Location.SourceTree != FilterTreeOpt.SourceTree)
                {
                    return false;
                }
            }
            ...
            {
                if (FilterTreeOpt?.NonSourceFile == null ||
                    !PathUtilities.Comparer.Equals(externalFileLocation.FilePath, FilterTreeOpt.NonSourceFile.Path))
                {
                    return false;
                }
            }
