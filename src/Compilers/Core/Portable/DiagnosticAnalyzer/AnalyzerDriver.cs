...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    ...
    internal abstract partial class AnalyzerDriver : IDisposable
    {
        ...
        /// </summary>
        private readonly ConcurrentSet<Suppression> _analyzerSuppressions;
        ...
        protected AnalyzerDriver(ImmutableArray<DiagnosticAnalyzer> analyzers, AnalyzerManager analyzerManager, Func<SyntaxTrivia, bool> isComment)
        {
            ...
            _analyzerSuppressions = _hasDiagnosticSuppressors ? new ConcurrentSet<Suppression>() : null;
        }
        ...
        private void AddSuppression(Suppression suppression)
            => _analyzerSuppressions.Add(suppression);
        ...
        private ImmutableArray<Diagnostic> ApplyAnalyzerSuppressionsCore(ImmutableArray<Diagnostic> reportedDiagnostics, Compilation compilation)
        {
            ...
            Debug.Assert(_analyzerSuppressions != null);
            ...
            if (_analyzerSuppressions.IsEmpty)
            {
                return reportedDiagnostics;
            }
            ...
            var analyzerSuppressionMessagesByDiagnostic = _analyzerSuppressions.GroupBy(s => s.SuppressedDiagnostic)
                .ToImmutableDictionary(g => g.Key, g => string.Join(", ", g.Select(s => $"{s.Descriptor.Id}: {s.Descriptor.Description}").Order()));

            foreach (var diagnostic in reportedDiagnostics)
            {
                if (analyzerSuppressionMessagesByDiagnostic.TryGetValue(diagnostic, out var suppressionMessage) &&
                    diagnostic is Diagnostic.SimpleDiagnostic simpleDiagnostic)
                {
                    Debug.Assert(suppressableDiagnostics.Contains(simpleDiagnostic));
                    Debug.Assert(!simpleDiagnostic.IsSuppressed);
                    builder.Add(simpleDiagnostic.WithSuppressionMessage(suppressionMessage));
                }
                else
                {
