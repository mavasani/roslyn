        ...
        private class PerAnalyzerState
        {
                ...
                where TAnalyzerStateData : AnalyzerStateData
            {
                ...
                if (pendingEntities.TryGetValue(analysisEntity, out state))
                {
                    ...
                    if (state != null)
                    {
                        Debug.Assert(state.StateKind == StateKind.InProcess, "An entity can be marked as processed only by its processing thread");
                        state.Free();
                        pool.Free(state);
                    }
                }
            ...
            public void OnCompilationEventGenerated_NoLock(CompilationEvent compilationEvent, AnalyzerActionCounts actionCounts)
            {
