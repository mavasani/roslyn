...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    ...
    internal partial class AnalysisResult
    {
        ...
        public void Update(AnalysisScope analysisScope, AnalyzerDriver driver)
        {
            foreach (var analyzer in analysisScope.Analyzers)
            {
                // Get reported diagnostics and add them to our local diagnostic caches.
                var syntaxDiagnostics = driver.GetLocalDiagnostics(analyzer, syntax: true);
                var semanticDiagnostics = driver.GetLocalDiagnostics(analyzer, syntax: false);
                var compilationDiagnostics = driver.GetNonLocalDiagnostics(analyzer);

                lock (_gate)
                {
                    if (syntaxDiagnostics.Length > 0 || semanticDiagnostics.Length > 0 || compilationDiagnostics.Length > 0)
                    {
                        UpdateLocalDiagnosticsCache_NoLock(analyzer, syntaxDiagnostics, ref _localSyntaxDiagnosticsOpt);
                        UpdateLocalDiagnosticsCache_NoLock(analyzer, semanticDiagnostics, ref _localSemanticDiagnosticsOpt);
                        UpdateNonLocalDiagnosticsCache_NoLock(analyzer, compilationDiagnostics);
                    }
        ...
        private void UpdateLocalDiagnosticsCache_NoLock(DiagnosticAnalyzer analyzer, ImmutableArray<Diagnostic> diagnostics, ref Dictionary<SyntaxTree, Dictionary<DiagnosticAnalyzer, List<Diagnostic>>> lazyLocalDiagnostics)
        {
        ...
        private void UpdateNonLocalDiagnosticsCache_NoLock(DiagnosticAnalyzer analyzer, ImmutableArray<Diagnostic> diagnostics)
        {
            if (diagnostics.IsEmpty)
            {
                return;
        ...
        public ImmutableArray<Diagnostic> GetCachedDiagnostics(AnalysisScope analysisScope, bool getLocalDiagnostics, bool getNonLocalDiagnostics)
        {
            lock (_gate)
            {
                return GetCachedDiagnostics_NoLock(analysisScope, getLocalDiagnostics, getNonLocalDiagnostics);
            }
        }

        private ImmutableArray<Diagnostic> GetCachedDiagnostics_NoLock(AnalysisScope analysisScope, bool getLocalDiagnostics, bool getNonLocalDiagnostics)
        {
