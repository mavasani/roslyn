            ...
            while (true)
            {
                ...
                lock (_executingTasksLock)
                {
                        ...
                        _treeTaskTokens[executingTreeTask.Item1] < newTaskToken)
                    {
                        ...
                        if (_executingCompilationTask != null)
                        {
                            SuspendAnalysis_NoLock(_executingCompilationTask.Item1, _executingCompilationTask.Item2);
        ...
        private async Task PopulateEventsCacheAsync(CancellationToken cancellationToken)
        {
            if (_compilation.EventQueue.Count > 0)
            {
                ...
                try
                {
                    ...
                    {
                        await _analysisState.OnCompilationEventGeneratedAsync(compilationEvent, driver, cancellationToken).ConfigureAwait(false);
                    }
                }
                finally
                {
