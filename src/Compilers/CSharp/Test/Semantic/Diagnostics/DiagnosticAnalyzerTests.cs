...
";
            ...
            VerifyExpectedGeneratedCodeAnalyzerDiagnostics(compilation, isGeneratedFile, generatedCodeConfiguration: null, reportDiagnosticsOnNestedFromContaining: false);

            //   (b) Non-error diagnostics reported on generated code from non-generated code are suppressed.
            VerifyExpectedGeneratedCodeAnalyzerDiagnostics(compilation, isGeneratedFile, generatedCodeConfiguration: null, reportDiagnosticsOnNestedFromContaining: true);

            // (2) Verify ConfigureGeneratedCodeAnalysis(enable: true).
            VerifyExpectedGeneratedCodeAnalyzerDiagnostics(compilation, isGeneratedFile, generatedCodeConfiguration: true, reportDiagnosticsOnNestedFromContaining: false);
            VerifyExpectedGeneratedCodeAnalyzerDiagnostics(compilation, isGeneratedFile, generatedCodeConfiguration: true, reportDiagnosticsOnNestedFromContaining: true);

            // (3) Verify ConfigureGeneratedCodeAnalysis(enable: false).
            VerifyExpectedGeneratedCodeAnalyzerDiagnostics(compilation, isGeneratedFile, generatedCodeConfiguration: false, reportDiagnosticsOnNestedFromContaining: false);
            VerifyExpectedGeneratedCodeAnalyzerDiagnostics(compilation, isGeneratedFile, generatedCodeConfiguration: false, reportDiagnosticsOnNestedFromContaining: true);
        }

        private static void VerifyExpectedGeneratedCodeAnalyzerDiagnostics(Compilation compilation, Func<string, bool> isGeneratedFileName, bool? generatedCodeConfiguration, bool reportDiagnosticsOnNestedFromContaining)
        {
            var analyzers = new DiagnosticAnalyzer[] { new GeneratedCodeAnalyzer(generatedCodeConfiguration, reportDiagnosticsOnNestedFromContaining) };
            var expected = GetExpectedGeneratedCodeAnalyzerDiagnostics(compilation, isGeneratedFileName, generatedCodeConfiguration, reportDiagnosticsOnNestedFromContaining);
            compilation.VerifyAnalyzerDiagnostics(analyzers, null, null, logAnalyzerExceptionAsDiagnostics: false, expected: expected);
        }

        private static DiagnosticDescription[] GetExpectedGeneratedCodeAnalyzerDiagnostics(Compilation compilation, Func<string, bool> isGeneratedFileName, bool? generatedCodeConfiguration, bool reportDiagnosticsOnNestedFromContaining)
        {
            var analyzers = new DiagnosticAnalyzer[] { new GeneratedCodeAnalyzer(generatedCodeConfiguration, reportDiagnosticsOnNestedFromContaining) };
            ...
            for (int i = 1; i <= 9; i++)
            {
                ...
                // File diagnostic
                squiggledText = "["; // first token in file.
                diagnosticArgument = file;
                line = !file.Contains("SourceFileWithAutoGeneratedComment") ? 2 : 3;
                column = 1;
                isGeneratedCode = isGeneratedFile;
                AddExpectedDiagnostics(builder, isGeneratedCode, squiggledText, line, column, generatedCodeConfiguration, reportDiagnosticsOnNestedFromContaining, diagnosticArgument);
                ...
                if (generatedCodeConfiguration == null || generatedCodeConfiguration == true)
                {
            ...
            AddExpectedDiagnostic(builder, GeneratedCodeAnalyzer.Summary.Id, squiggledText: null, line: 1, column: 1, arguments: new[] { arg1, arg2 });

            return builder.ToArrayAndFree();
        }

        private static void AddExpectedDiagnostics(
            ArrayBuilder<DiagnosticDescription> builder,
            bool isGeneratedCode,
            string squiggledText,
            int line,
            int column,
            bool? generatedCodeConfiguration,
            bool reportDiagnosticsOnNestedFromContaining,
            params string[] arguments)
        {
            ...
            if (!isGeneratedCode || generatedCodeConfiguration == true)
            {
            ...
            if (!isGeneratedCode || generatedCodeConfiguration == null || generatedCodeConfiguration == true)
            {
                var diagnostic = Diagnostic(GeneratedCodeAnalyzer.Error.Id, squiggledText).WithArguments(arguments).WithLocation(line, column);
                builder.Add(diagnostic);
            }
        }
