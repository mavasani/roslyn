...
namespace Microsoft.CodeAnalysis.Editor.CSharp.UnitTests.RemoveUnusedExpressions
{
    public partial class RemoveUnusedExpressionsTests : AbstractCSharpDiagnosticProviderBasedUserDiagnosticTest
    {
        ...
        // Ensure that we explicitly test missing IDE0057, which has no corresponding code fix (non-fixable diagnostic).
        ...
        private Task TestDiagnosticMissingAsync(string initialMarkup, string optionName)
            => TestDiagnosticMissingAsync(initialMarkup, GetOptions(optionName));
        ...
        private Task TestDiagnosticsAsync(string initialMarkup, string optionName, params DiagnosticDescription[] expectedDiagnostics)
            => TestDiagnosticsAsync(initialMarkup, GetOptions(optionName), expectedDiagnostics);
        ...
        private async Task TestDiagnosticMissingWithAllOptionsAsync(string initialMarkup)
        {
            foreach (var options in new[] { PreferDiscard, PreferUnusedLocal })
            {
                await TestDiagnosticMissingAsync(initialMarkup, options);
            }
        }
...
{{
    ...
    C M2() => null;
        ...
        [InlineData(nameof(PreferDiscard), "_")]
        [InlineData(nameof(PreferUnusedLocal), "var unused")]
        public async Task LValueFlowCapture_Assignment_ControlFlowInAssignedValue_02(string optionName, string fix)
        {
            await TestMissingInRegularAndScriptAsync(
@"class C
{
...
{{
    ...
    C M2() => null;
        ...
        [InlineData(nameof(PreferDiscard), "_")]
        [InlineData(nameof(PreferUnusedLocal), "var unused")]
        public async Task LValueFlowCapture_DeconstructionAssignment_ControlFlowInAssignedValue_02(string optionName, string fix)
        {
            await TestMissingInRegularAndScriptAsync(
@"class C
{
...
class C
{
    ...
    void M2(Action a) => a();
        ...
        [InlineData(nameof(PreferDiscard))]
        [InlineData(nameof(PreferUnusedLocal))]
        public async Task UseInLocalFunction_Nested(string optionName)
        {
            await TestMissingInRegularAndScriptAsync(
@"using System;
