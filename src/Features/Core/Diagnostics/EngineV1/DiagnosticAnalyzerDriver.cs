        ...
        private CompilationWithAnalyzers GetCompilationWithAnalyzers(Compilation compilation)
        {
            ...
            return _owner.HostAnalyzerManager.GetOrCreateCompilationWithAnalyzers(_project, p =>
            {
                var analyzers = _owner
                    .GetAnalyzers(p)
                    .Where(a => !CompilationWithAnalyzers.IsDiagnosticAnalyzerSuppressed(a, compilation.Options, _onAnalyzerException))
                    .ToImmutableArray()
                    .Distinct();
                return new CompilationWithAnalyzers(compilation, analyzers, _analyzerOptions, _onAnalyzerException, allowConcurrentAnalysis: false, logAnalyzerExecutionTime: true);
            });
        }

        public async Task<AnalyzerActionCounts> GetAnalyzerActionsAsync(DiagnosticAnalyzer analyzer)
        {
