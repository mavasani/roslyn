            ...
            if (compilationWithAnalyzers == null || !analyzer.SupportAnalysisKind(kind))
            {
                if (kind == AnalysisKind.Syntax)
                {
                    Logger.Log(FunctionId.Diagnostics_SyntaxDiagnostic,
                        (r, d, a, k) => $"Driver: {r != null}, {d.Id}, {d.Project.Id}, {a}, {k}", compilationWithAnalyzers, (Document)textDocument, analyzer, kind);
                }
                ...
                case AnalysisKind.Syntax:
                    ...
                    else
                    {
                        ...
                        if (textDocument.Kind == TextDocumentKind.AdditionalDocument)
                        {
                            ...
                            {
                                diagnostics = await compilationWithAnalyzers.GetAnalyzerNonSourceFileDiagnosticsAsync(additionalFile, singleAnalyzer, cancellationToken).ConfigureAwait(false);
                                break;
                            }
                        }
