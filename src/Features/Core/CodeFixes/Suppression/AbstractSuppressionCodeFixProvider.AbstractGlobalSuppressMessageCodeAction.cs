...
namespace Microsoft.CodeAnalysis.CodeFixes.Suppression
{
    internal abstract partial class AbstractSuppressionCodeFixProvider : ISuppressionFixProvider
    {
        internal abstract class AbstractGlobalSuppressMessageCodeAction : AbstractSuppressionCodeAction
        {
            ...
            protected AbstractGlobalSuppressMessageCodeAction(AbstractSuppressionCodeFixProvider fixer, string workflowState, Project project)
                : base (fixer, title: string.Format(FeaturesResources.TriageDiagnostic, workflowState), workflowState: workflowState)
            {
                Contract.ThrowIfNull(workflowState);
                _project = project;
            }
                ...
                while (suppressionsDoc == null)
                {
                    ...
                    foreach (var d in _project.Documents)
                    {
                        if (d.Name == suppressionsFileName)
                        {
                            ...
                            if (r.ChildNodes().All(n => Fixer.IsValidTopLevelNodeForSuppressionFile(n)))
                            {
                                suppressionsDoc = d;
                                break;
                            }
            ...
            internal bool IsSuppressionFix_TestOnly => WorkflowState == WellKnownWorkflowStates.SuppressedFalsePositive ||
